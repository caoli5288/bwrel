...
public class NewItemShop {
    ...
    public void openCategoryInventory(Player player) {
        ...
        Inventory inventory = Bukkit.createInventory(player, size, Main.local("ingame.shop.name"));
        ...
        slimeMeta.setDisplayName(Main.local("ingame.shop.oldshop"));
        ...
        if (game != null) {
            if (game.getPlayerSettings(player).oneStackPerShift()) {
                ...
                meta.setDisplayName(ChatColor.AQUA + Main.local("default.currently") + ": " + ChatColor.WHITE
                        + Main.local("ingame.shop.onestackpershift"));
                ...
                meta.setDisplayName(ChatColor.AQUA + Main.local("default.currently") + ": " + ChatColor.WHITE
                        + Main.local("ingame.shop.fullstackpershift"));
                meta.setLore(new ArrayList<String>());
                stack.setItemMeta(meta);
            }
    ...
    private void openBuyInventory(MerchantCategory category, Player player, Game game) {
        ...
        Inventory buyInventory = Bukkit.createInventory(player, invSize, Main.local("ingame.shop.name"));
    ...
    private void handleBuyInventoryClick(InventoryClickEvent ice, Game game, Player player) {
        ...
        if (ice.getRawSlot() < sizeCategories) {
            ...
            if (!this.hasEnoughRessource(player, trade)) {
                player
                        .sendMessage(ChatHelper.with(ChatColor.RED + Main.local("errors.notenoughress")));
                return;
            }

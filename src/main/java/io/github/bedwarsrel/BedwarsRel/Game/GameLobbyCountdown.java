    ...
    public void run() {
        ...
        if (!this.game.isStartable()) {
            if (!this.game.hasEnoughPlayers()) {
                this.game.broadcast(ChatColor.RED + Main.local("lobby.cancelcountdown.not_enough_players"),
                        players);
            } else if (!this.game.hasEnoughTeams()) {
                this.game.broadcast(ChatColor.RED + Main.local("lobby.cancelcountdown.not_enough_teams"),
                        players);
            }

...
public class PlayerStatisticManager {
    ...
    public void initializeDatabase() {
        Main.getInstance().getServer().getConsoleSender().sendMessage(
                ChatHelper.with(ChatColor.GREEN + "Loading Statistics from Database ..."));
    ...
    public void storeStatistic(PlayerStatistic statistic) {
        if (Main.getInstance().getStatisticStorageType() == StorageType.YAML) {
            ...
            Main.execute(() -> storeDatabaseStatistic(statistic));
        }
    }
    ...
    private synchronized void storeYamlStatistic(PlayerStatistic statistic) {
        ...
        try {
            ...
            Main.getInstance().getBugsnag().notify(ex);
            Main.getInstance().getServer().getConsoleSender().sendMessage(ChatHelper.with(
    ...
    public PlayerStatistic getStatistic(OfflinePlayer player) {
        ...
        PlayerStatistic statistic = this.playerStatistic.get(player);
        if (statistic == null) {
            statistic = new PlayerStatistic(player);
            statistic.load();
            playerStatistic.put(player, statistic);
        }
        return statistic;
    }
    ...
    private void loadYml(File ymlFile) {
        try {
            Main.getInstance().getServer().getConsoleSender().sendMessage(
                    ChatHelper.with(ChatColor.GREEN + "Loading statistics from YAML-File ..."));
